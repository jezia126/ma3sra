		...
		<div class="toolbar-nav" >   
			<div class="row">
				...
				<div class="col-md-8 col-8  text-right"> 	
					<div class="btn-group">
						@if($access['is_add'] ==1)
						<a href="{{ url('gestionlotdestockageolive/create?return='.$return) }}" class="btn  btn-sm btn-primary"  
			...
			<!-- Table Grid -->
 			...
 			{!! Form::open(array('url'=>'gestionlotdestockageolive?'.$return, 'class'=>'form-horizontal m-t' ,'id' =>'SximoTable' )) !!}
							 ...
							 @if($field['view'] =='1')
							 	...
							 	@if(SiteHelpers::filterColumn($limited ))
								  ...
								  <td align="{{ $field['align'] }}" width=" {{ $field['width'] }}"  {!! $addClass !!} >	
									...
									<?php		if($field['field']=="tiers_crediteur"){
		
												$rows=\DB::table('tb_reception_olive')->where('id',92)->first();
												echo($rows->tiers);
						 ...
						 <td>
							 	...
							 	<div class="dropdown">
								  ...
								  <ul class="dropdown-menu">
									...
									<li class="nav-item"><a href="{{ url('gestionlotdestockageolive/'.$row->id.'?return='.$return)}}" class="nav-link tips" title="{{ __('core.btn_view') }}"> {{ __('core.btn_view') }} </a></li>
									@endif
									@if($access['is_edit'] ==1)
									<li class="nav-item"><a  href="{{ url('gestionlotdestockageolive/'.$row->id.'/edit?return='.$return) }}" class="nav-link  tips" title="{{ __('core.btn_edit') }}"> {{ __('core.btn_edit') }} </a></li>
									@endif
									<div class="dropdown-divider"></div>
									@if($access['is_remove'] ==1)
										<li class="nav-item"><a href="javascript://ajax"  onclick="SximoDelete();" class="nav-link  tips" title="{{ __('core.btn_remove') }}">
