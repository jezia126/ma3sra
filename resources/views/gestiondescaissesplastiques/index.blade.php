		...
		<div class="toolbar-nav" >   
			<div class="row">
				...
				<div class="col-md-8 col-8  text-right"> 	
					<div class="btn-group">
						@if($access['is_add'] ==1)
						<a href="{{ url('gestiondescaissesplastiques/create?return='.$return) }}" class="btn  btn-sm btn-primary"  
			...
			<!-- Table Grid -->
 			...
 			{!! Form::open(array('url'=>'gestiondescaissesplastiques?'.$return, 'class'=>'form-horizontal m-t' ,'id' =>'SximoTable' )) !!}
		        ...
		        <tbody>        						
		            @foreach ($rowData as $row)
		                <tr>
							<td class="thead"> {{ ++$i }} </td>
							<td class="tcheckbox">
								<input type="checkbox" class="ids filled-in" name="ids[]" value="{{ $row->code }}" id="val-{{ $row->code }}" /> 
								<label for="val-{{ $row->code }}"></label>
							</td>
						 ...
						 <td>
							 	...
							 	<div class="dropdown">
								  ...
								  <ul class="dropdown-menu">
									...
									<li class="nav-item"><a href="{{ url('gestiondescaissesplastiques/'.$row->code.'?return='.$return)}}" class="nav-link tips" title="{{ __('core.btn_view') }}"> {{ __('core.btn_view') }} </a></li>
									@endif
									@if($access['is_edit'] ==1)
									<li class="nav-item"><a  href="{{ url('gestiondescaissesplastiques/'.$row->code.'/edit?return='.$return) }}" class="nav-link  tips" title="{{ __('core.btn_edit') }}"> {{ __('core.btn_edit') }} </a></li>
									@endif
									<div class="dropdown-divider"></div>
									@if($access['is_remove'] ==1)
										<li class="nav-item"><a href="javascript://ajax"  onclick="SximoDelete();" class="nav-link  tips" title="{{ __('core.btn_remove') }}">
