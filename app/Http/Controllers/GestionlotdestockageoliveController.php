...
use App\Models\Gestionlotdestockageolive;
use Illuminate\Http\Request;
use Illuminate\Pagination\LengthAwarePaginator as Paginator;
use Validator, Input, Redirect ; 


class GestionlotdestockageoliveController extends Controller {

	protected $layout = "layouts.main";
	protected $data = array();	
	public $module = 'gestionlotdestockageolive';
	static $per_page	= '10';

	public function __construct()
	{		
		parent::__construct();
		$this->model = new Gestionlotdestockageolive();	
		
		$this->info = $this->model->makeInfo( $this->module);	
		$this->data = array(
			'pageTitle'	=> 	$this->info['title'],
			'pageNote'	=>  $this->info['note'],
			'pageModule'=> 'gestionlotdestockageolive',
			'return'	=> self::returnUrl()
			
		);
		
	}
	public function getgestionlotstockageolive($id)
	{
        
		$gestionlotdestockageolive=Gestionlotdestockageolive::find($id);
		 	response()->json($gestionlotdestockageolive);

	//return Ficheclient::all();
	$gestionlotdestockageolive=Gestionlotdestockageolive::find($id);
		if(is_null($gestionlotdestockageolive)){
			return response()->json(['message' => 'lot de stockage olive introuvable']);

		}
		return response()->json(Gestionlotdestockageolive::find($id),200);
	...
	public static function display(  )
	{
		...
		$model  = new Gestionlotdestockageolive();
		$info = $model::makeInfo('gestionlotdestockageolive');
		$data = array(
		...
		if($mode == 'view')
		{
			...
			if($row)
			{
				...
				$data['id'] = $id;
				return view('gestionlotdestockageolive.public.view',$data);			
			}			
		} 
		else {
			...
			return view('gestionlotdestockageolive.public.index',$data);	
		}
