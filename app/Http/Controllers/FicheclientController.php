...
class FicheclientController extends Controller {
    ...
    public $module = 'ficheclient';
    static $per_page    = '10';

    public function __construct()
    {       
        parent::__construct();
        $this->model = new Ficheclient();   
        
        $this->info = $this->model->makeInfo( $this->module);   
        $this->data = array(
            'pageTitle' =>  $this->info['title'],
            'pageNote'  =>  $this->info['note'],
            'pageModule'=> 'ficheclient',
            'return'    => self::returnUrl()
            
        );
        
    }
	public function getlotdestockageolive ($id){
        
		$ficheclient=FicheClient::find($id);
		 	response()->json($ficheclient);

	//return Ficheclient::all();
	$client=Ficheclient::find($id);
		if(is_null($client)){
			return response()->json(['message' => 'client introuvable']);

		}
		return response()->json(Ficheclient::find($id),200);
    ...
    function edit( Request $request , $id ) 
    {
        ...
        if($this->access['is_edit'] ==0 )
            return redirect('dashboard')->with('message',__('core.note_success_action'))->with('status','error');
    ...
    public static function display(  )
    {
        ...
        $model  = new Ficheclient();
        $info = $model::makeInfo('ficheclient');
        $data = array(
        ...
        if($mode == 'view')
        {
            ...
            if($row)
            {
                ...
                $data['id'] = $id;
                return view('ficheclient.public.view',$data);           
            }           
        } 
        else {
            ...
            return view('ficheclient.public.index',$data);  
        }
